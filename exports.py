...
def generate_exports(section_id):
    section = Section.query.get(section_id)
    if not section:
        return None, None

    timestamp = datetime.now().strftime("%Y%m%d_%H%M")
    filename_base = f"{section.name.replace(' ', '_')}_{timestamp}"
    ...
    with pd.ExcelWriter(xlsx_path, engine='openpyxl') as writer:
        for theme_id in range(1, 5):
            # Prendre la dernière réponse pour cette section et ce thème
            # On cherche la réponse liée au template actif ou la plus récente
            response = FormResponse.query.join(FormTemplate).filter(
                FormResponse.section_id == section_id,
                FormTemplate.theme == theme_id
            ).order_by(FormResponse.last_updated.desc()).first()
    ...
    for theme_id in range(1, 5):
        ...
        pdf.cell(0, 10, f"{section.name} - {title}", new_x="LMARGIN", new_y="NEXT", align='C')
        pdf.ln(10)
        ...
        if response:
            ...
            if template.structure['type'] == 'budget':
                ...
                # Simplifié pour le moment : un tableau ou une liste
                # Pour Bureau et Tables dynamiques, on va faire une liste
