...
def dashboard():
    if 'section_name' not in session:
        return redirect(url_for('index'))
    return render_template('dashboard.html', section_name=session['section_name'])
...
def form_view(theme_id):
    ...
    response = storage.get_response(session['section_name'], theme_id)
    # On ne charge les données que si elles correspondent à la version actuelle du template
    # Comme demandé : "s'il bascule en v2, alors il devra tout remplir de nouveau"
    data = {}
    if response and response.get('template_version') == template['version']:
        data = response['data']
